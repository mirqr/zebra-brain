    ...
    for file in os.listdir(folder_path):
        if file.endswith(".nrrd"):
            ...
            nii_path = os.path.join(
                folder_path, file.replace(".nrrd", ".nii.gz")
...
def convert_to_orig(file):
    file_converted_orig = file.replace(
        ".nii.gz", "+orig"
    )  # default it creates .tlrc file
    # 3dcopy
...
def convert_file_seconds(filename):
    ...
    with open(filename, "r") as file:
        for line in file:
            ...
            converted_line = re.sub(
                r"\b\d+\.\d+\b",
                lambda x: convert_seconds_to_hms(float(x.group())),
                line.strip(),
            )
    ...
    # same output_file_path = filename
